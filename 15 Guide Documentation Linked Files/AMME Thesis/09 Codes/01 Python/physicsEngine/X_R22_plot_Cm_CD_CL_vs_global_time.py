...
if flag_whether_running_xplane == 1:
    ...
    poststall_on = df.iloc[150:, 11]  # assuming the 11th column is poststall_on
    ...
    poststall_on = df.iloc[:, 11]  # assuming the 11th column is poststall_on
...
ax1.plot(global_time, x_flow_sep, label=r'$X$ vs. global_time', color=color2)
...
if plot_alpha_unit_flag == 0:
    ...
    ax2 = ax1.twinx()
    ax2.plot(global_time, alpha_in_deg, label=r'$\alpha$ vs. global_time', color=color1, linestyle='--')
    ax2.plot(global_time, alpha_crit_1_in_deg, label=r'$\alpha_{crit}$ (interp 1) vs. global_time', color=color3, linestyle='-.')  
    ax2.plot(global_time, alpha_crit_2_in_deg, label=r'$\alpha_{crit}$ (interp 2) vs. global_time', color=color4, linestyle='-.')  
    ax2.plot(global_time, alpha_crit_3_in_deg, label=r'$\alpha_{crit}$ (interp 3) vs. global_time', color=color5, linestyle='-.')  
    ax2.plot(global_time, 2 * CL, label=r'2 $C_L$ vs. global_time', color=color6, linestyle='-') # CL is multiplied by 2 and displayed on ax2 scale
    ax2.plot(global_time, poststall_on, label='poststall_on vs. global_time', color=color7, linestyle='-') # prestall/poststall indicator
    ax2.plot(global_time, 2 * Cm, label=r'2 $C_m$ vs. global_time', color=color8, linestyle='-') # Cm
    ax2.plot(global_time, 5 * CD, label=r'5 $C_D$ vs. global_time', color=color9, linestyle='-') # CD
...
if plot_alpha_unit_flag == 1:
    ...
    ax2 = ax1.twinx()
    ax2.plot(global_time, alpha_in_rad, label=r'$\alpha$ vs. global_time', color=color1, linestyle='--')
    ax2.plot(global_time, alpha_crit_1_in_rad, label=r'$\alpha_{crit}$ (interp 1) vs. global_time', color=color3, linestyle='-.')
    ax2.plot(global_time, alpha_crit_2_in_rad, label=r'$\alpha_{crit}$(interp 2) vs. global_time', color=color4, linestyle='-.')  
    ax2.plot(global_time, alpha_crit_3_in_rad, label=r'$\alpha_{crit}$ (interp 3) vs. global_time', color=color5, linestyle='-.') 
    ax2.plot(global_time, 2 * CL, label=r'2 * $C_L$ vs. global_time', color=color6, linestyle='-') # CL is multiplied by 2 and displayed on ax2 scale
    ax2.plot(global_time, poststall_on, label='poststall_on vs. global_time', color=color7, linestyle='-') # prestall/poststall indicator
    ax2.plot(global_time, 2 * Cm, label=r'2 * $C_m$ vs. global_time', color=color8, linestyle='-') # Cm
    ax2.plot(global_time, 5 * CD, label=r'5 * $C_D$ vs. global_time', color=color9, linestyle='-') # CD
    ax2.set_ylabel(r'$\alpha$ [rad]', color=color1)
    ax2.tick_params(axis='y', labelcolor=color1)

# Title and legends
plt.title(r'$X$ and $C_L$, $C_m$, $C_D$ vs. Global Time')
fig.tight_layout()
fig.legend(loc='upper right', bbox_to_anchor=(0.9, 0.9))
plt.savefig('C:\\Users\\Richard\\00 Richard Apps\\Steam\\steamapps\\common\\X-Plane 11\\richard_checked_terms\\X_flow_sep_CL_Cm_CD_vs_global_time.png')  # Save as PNG
plt.show()
