...
if plot_alpha_unit_flag == 0:
    ...
    ax1.plot(alpha_in_deg, x_flow_sep, label='X_flow_sep vs. alpha', color=color2)
    ...
    ax2.plot(alpha_in_deg, CL, label='CL vs. alpha', color=color6, linestyle='-')
    ax2.plot(alpha_in_deg, poststall_on, label='poststall_on vs. alpha', color=color7, linestyle='-')
    ax2.set_ylabel('CL [-]', color=color1)
...
if plot_alpha_unit_flag == 1:
    ...
    # Plot X_flow_sep on the left y-axis
    ax1.plot(alpha_in_rad, x_flow_sep, label='X_flow_sep vs. alpha', color=color2)
    ...
    ax2.plot(alpha_in_rad, CL, label='CL vs. alpha', color=color6, linestyle='-')
    ax2.plot(alpha_in_rad, poststall_on, label='poststall_on vs. alpha', color=color7, linestyle='-')
    ax2.set_ylabel('CL [-]', color=color1)
    ax2.tick_params(axis='y', labelcolor=color1)

# Title and legends
plt.title('X_flow_sep and CL vs. alpha')
fig.tight_layout()
fig.legend(loc='upper right', bbox_to_anchor=(0.9, 0.3))
plt.savefig('C:\\Users\\Richard\\00 Richard Apps\\Steam\\steamapps\\common\\X-Plane 11\\richard_checked_terms\\X_flow_sep_CL_vs_alpha.png')  # Save as PNG
plt.show()
