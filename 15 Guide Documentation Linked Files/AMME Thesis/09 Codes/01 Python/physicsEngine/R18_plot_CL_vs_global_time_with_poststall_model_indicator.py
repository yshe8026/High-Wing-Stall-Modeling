...
if plot_alpha_unit_flag == 0:
    ...
    ax2.plot(global_time, 2 * CL, label='2 * CL vs. global_time', color=color6, linestyle='-') # CL is multiplied by 2 and displayed on ax2 scale
    ax2.plot(global_time, poststall_on, label='poststall_on vs. global_time', color=color7, linestyle='-') # CL is multiplied by 2 and displayed on ax2 scale
...
plt.savefig('C:\\Users\\Richard\\00 Richard Apps\\Steam\\steamapps\\common\\X-Plane 11\\richard_checked_terms\\X_flow_sep_alpha_vs_global_time.png')  # Save as PNG
plt.show()
