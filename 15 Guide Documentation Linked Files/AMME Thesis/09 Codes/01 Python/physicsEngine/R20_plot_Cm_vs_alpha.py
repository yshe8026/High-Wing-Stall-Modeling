...
if plot_alpha_unit_flag == 0:
    ...
    ax2.plot(alpha_in_deg, Cm, label='Cm vs. alpha', color=color8, linestyle='-')
    ax2.set_ylabel('Cm [-]', color=color1)
...
if plot_alpha_unit_flag == 1:
    ...
    ax2.plot(alpha_in_rad, Cm, label='Cm vs. alpha', color=color8, linestyle='-')
    ax2.set_ylabel('Cm [-]', color=color1)
    ax2.tick_params(axis='y', labelcolor=color1)

# Title and legends
plt.title('X_flow_sep and Cm vs. alpha')
fig.tight_layout()
fig.legend(loc='upper right', bbox_to_anchor=(0.9, 0.3))
plt.savefig('C:\\Users\\Richard\\00 Richard Apps\\Steam\\steamapps\\common\\X-Plane 11\\richard_checked_terms\\X_flow_sep_Cm_vs_alpha.png')  # Save as PNG
plt.show()
